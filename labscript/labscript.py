...
class Device(object):
    ...
    def set_property(self, name, value, location=None, overwrite=True): # TODO: dirty hack..
...
def generate_code():
    ...
    if compiler.hdf5_run_filename is None:
        raise LabscriptError('hdf5 file for compilation not set. Please call labscript_init')
    elif not os.path.exists(compiler.hdf5_run_filename):
        with h5py.File(compiler.hdf5_run_filename ,'w') as hdf5_file:
            hdf5_file.create_group('globals')

    if compiler.hdf5_shot_template_filename is None:
        raise LabscriptError('hdf5 file for shot not set. Please call labscript_init')

    # Copy main file as a starting point
    shutil.copy(compiler.hdf5_run_filename, compiler.hdf5_shot_template_filename)

    with h5py.File(compiler.hdf5_shot_template_filename, 'a') as hdf5_shot_template_file:
        try:
            del hdf5_shot_template_file['shot_templates']
            if "connection table" in hdf5_shot_template_file:
                del hdf5_shot_template_file['connection table']
            hdf5_shot_template_file.create_group('devices')
            hdf5_shot_template_file.create_group('calibrations')
        except ValueError as err:
            # Group(s) already exist - this is not a fresh h5 file, we cannot compile with it:
            raise ValueError('The HDF5 file %s already contains compilation data '%compiler.hdf5_shot_template_filename +
        ...
        for device in compiler.inventory:
            if device.parent_device is None:
                device.generate_code(hdf5_shot_template_file)

        save_time_markers(hdf5_shot_template_file)
        generate_connection_table(hdf5_shot_template_file)
        write_device_properties(hdf5_shot_template_file)
        generate_wait_table(hdf5_shot_template_file)
        save_labscripts(hdf5_shot_template_file)

        # Save shot properties:
        group = hdf5_shot_template_file.create_group('shot_properties')
        set_attributes(group, compiler.shot_properties)
    
    with h5py.File(compiler.hdf5_run_filename, 'a') as hdf5_run_file:
        try:
            hdf5_run_file['shot_templates'][compiler.shot_template_name] = h5py.ExternalLink(compiler.hdf5_shot_template_filename, "/")

            if "connection table" not in hdf5_run_file:
                print("Generate con table for main file...")
                generate_connection_table(hdf5_run_file)
        except ValueError as err:
            # Shot already exists
            print(err)
            raise ValueError('The HDF5 file %s already contains a shot named %s '%(compiler.hdf5_shot_template_filename, compiler.shot_template_name))
...
def start(shot_name = "main"):
    ...
    composition_compiler.inventory = compiler.inventory.copy()
    compiler.shot_template_name = shot_name
    compiler.hdf5_shot_template_filename = f"{compiler.shot_template_folder}/{shot_name}.h5"

    if compiler.max_delay >= 0:
        return compiler.max_delay
...
def stop(t, target_cycle_time=None, cycle_time_delay_after_programming=False):
    ...
    # reset compiler
    # compiler.inventory = composition_compiler.inventory.copy()
    compiler.shot_template_name = None
    compiler.hdf5_shot_template_filename = None
    compiler.wait_table = {}
...
def executor(exec_func):

    compiler.executor_script = exec_func

def run(shot_name):

    print(f"Start execution of {shot_name}")

    executor_data.shot_callback(executor_data.runfile_path, shot_name, executor_data.shot_id)
    # shot_callback(self, runfile_path, shot_name, shot_id)
    executor_data.shot_id += 1

    print(f"Finish execution of {shot_name}")

def init_executor(shot_callback, runfile_path):

    executor_data.shot_callback = shot_callback
    executor_data.shot_id = 0
    executor_data.runfile_path = runfile_path
...
def labscript_init(hdf5_run_filename, shot_template_folder, shot_folder, labscript_file=None, new=False, overwrite=False, load_globals_values=True):
    ...
    if new:
        ...
        if os.path.exists(hdf5_run_filename) and overwrite:
            os.unlink(hdf5_run_filename)
    elif not os.path.exists(hdf5_run_filename):
        raise LabscriptError('Provided hdf5 filename %s doesn\'t exist.'%hdf5_run_filename)
    # TO_DELETE:runmanager-batchompiler-agnostic 
    elif load_globals_values:
        load_globals(hdf5_run_filename) 
    # END_DELETE:runmanager-batchompiler-agnostic 
    
    compiler.hdf5_run_filename = os.path.abspath(hdf5_run_filename)
    compiler.shot_folder = os.path.abspath(shot_folder)
    compiler.shot_template_folder = os.path.abspath(shot_template_folder)
...
def labscript_composition_init(hdf5_run_filename, shot_template_folder, shot_folder, labscript_file=None, new=False, overwrite=False, load_globals_values=True):
    """Initialises labscript and prepares for compilation.
    ...
    """
    # save the builtins for later restoration in labscript_cleanup
    composition_compiler._existing_builtins_dict = _builtins_dict.copy()
    ...
    composition_compiler.hdf5_run_filename = os.path.abspath(hdf5_run_filename)
    composition_compiler.shot_folder = os.path.abspath(shot_folder)
    composition_compiler.shot_template_folder = os.path.abspath(shot_template_folder)
    if labscript_file is None:
        import __main__
        labscript_file = __main__.__file__
    composition_compiler.labscript_file = os.path.abspath(labscript_file)
...
def labscript_cleanup():
    ...
    with h5py.File(compiler.hdf5_run_filename, 'a') as hdf5_run_file:
        hdf5_run_file.attrs['executor_script'] = compiler.executor_script
    ...
    compiler.inventory = []
    compiler.hdf5_run_filename = None
    compiler.shot_template_name = None
    compiler.shot_folder = None
    compiler.shot_template_folder = None
    compiler.hdf5_shot_template_filename = None
    ...
    compiler.shot_properties = {}
    compiler.max_delay = -1
    compiler.executor_script = """
from labscript import run

run("main")
    """
    composition_compiler.inventory = []
...
class compiler(object):
    ...
    hdf5_run_filename = None
    shot_template_name = None
    shot_folder = None
    shot_template_folder = None
    hdf5_shot_template_filename = None
    start_called = False
    wait_table = {}
    wait_monitor = None
...
class composition_compiler(object):
    # All defined devices:
    inventory = []

class executor_data(object):
    shot_callback = None
    shot_id = 0
    runfile_path = None
